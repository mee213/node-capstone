...
app.get('/addLabor', (req, res, next) => {
  ...
  request(
    { method: 'GET',
      uri: `${req.protocol}://${req.hostname}${(process.env.NODE_ENV != 'production' ? ':' + PORT : '')}/laborWeeks/${week_id}`,
      json: true},
    (error, response, body) => {
      if (error) {
        return next(error);
...
app.get('/addSales', (req, res, next) => {
  ...
  request(
    { method: 'GET',
      uri: `${req.protocol}://${req.hostname}${(process.env.NODE_ENV != 'production' ? ':' + PORT : '')}/salesWeeks/${week_id}`,
      json: true},
    (error, response, body) => {
      if (error) {
        return next(error);
